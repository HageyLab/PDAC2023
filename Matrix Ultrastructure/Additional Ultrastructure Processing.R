...
temp_coords <- matrix(ncol=length(loaded_metadata["Patient.ID",]), nrow = 1)
colnames(temp_coords) <- loaded_metadata["Patient.ID",]
rownames(temp_coords) <- c("Processed.DFS")
...
for (patient in 1:length(loaded_metadata['DFS',])) # If disease free survival is not listed, disease free survival is by definition equivalent to overall survival
{
  if (is.na(loaded_metadata['DFS',patient]))
  {
...
colData(us_object)$neoadjuvant_chemo <- as.character(loaded_metadata['Neoadjuvant.Category',as.character(loadedkey['Patient.ID',])])
...
colnames(temp_coords) <- loaded_metadata["Patient.ID",]
rownames(temp_coords) <- c("Median.Pseudotime","US.Variance")
colnames(loaded_metadata) <- loaded_metadata["Patient.ID",]
loaded_metadata <- rbind(loaded_metadata,temp_coords)
for (namedpatient in loaded_metadata["Patient.ID",])
{
  loaded_metadata["Median.Pseudotime",namedpatient] <- median(as.numeric(pseudotime[colData(us_object)$patient_ID == namedpatient]))
  ...
  loaded_metadata["US.Variance",namedpatient] <- sum(ecm_variance, na.rm = TRUE)
}
