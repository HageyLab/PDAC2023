...
patient_groups <- c(unique(loaded_metadata["Survivorship.Category",]),unique(loaded_metadata["Neoadjuvant.Category",]),unique(loaded_metadata["Adjuvant.Category",]),unique(loaded_metadata["Lymphovascular.Category",]),unique(loaded_metadata["Perineural.Category",]))
...
continuous_comparisons <- c("OS","Processed.DFS","Tumor.size")
# Specify ordinal categories of interest
ordinal_comparisons <- c("Grade","AJCC.Stage")
# Specify survival-positive and survival-negative categories
survival_positive_categories <- c("OS","Processed.DFS")
survival_negative_categories <- c("Grade","AJCC.Stage","Tumor.size")
